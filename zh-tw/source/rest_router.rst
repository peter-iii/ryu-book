...
下面的例子介紹如何建立拓璞，每個交換器（路由器）或 address 的新增或刪除，及確認 host 之間的連線狀況確認。
...
接著，每一個 host 刪除原先自動配置的 IP 位址，並設定新的 IP 位址。
...
最後，在操作 controller 的 xterm 上啟動 rest_router。
...
首先，設定交換器s1的位址「172.16.20.1/24」和「172.16.30.30/24」。

.. NOTE::

    接下來的說明中所使用的 REST API 請參考本章結尾的「 `REST API 一覽表`_ 」以取得更詳細的資料。
...
接著，設定交換器s2的位址為「172.16.10.1/24」、「172.16.30.1/24」和「192.168.10.1/24」。
...
接著，設定交換器 s3 的位址為「192.168.30.1/24」和「192.168.10.20/24」。
...
交換器的IP位址已經被設定完成，對每一個 host 新增預設的閘道。
...
為了路由器s2，設定路由器s3的靜態路由為 (192.168.30.0/24)。
...
確認路由器s2的設定。這邊可以看到原先指向路由器s3的靜態路由已經被刪除了。
...
在這個狀態下，使用 ping 來確認連結狀態。在 h2 上向 h3 執行 ping 會發現無法通過連接測試，這是因為我們已經刪除了路由的關係。
...
刪除已經設定在路由器 s1 上的 address 「172.16.20.1/24」。
...
接下來的例子將建立一個網路拓璞，使用VLAN來分割 tenant 的使用，對各個交換器（路由器）的位址或路由進行新增和刪除，並確認每一個 host 之間的連通狀況。
...
參數          參數值        説明
============ ============ ===========================================
topo         linear,3,2   3台交換器連接成一個直線的網路拓璞
                          (每個交換器連接兩台host)
...
之後，設定每一個 host 的 VLAN ID 和 IP 位址。
...
最後，在連線 controller 的 xterm 上啟動 rest_router。
...
首先，設定路由器s1的位址為「172.16.20.1/24」和「10.10.10.1/24」。
接著 VLAN ID 的設定也是必要的。
...
接下來，設定路由器s2的位址為「192.168.30.1/24」和「10.10.10.2/24」。
...
然後，設定路由器s3的位址為「172.16.20.1/24」和「10.10.10.3/24」。
...
路由器的IP位址已經設定好，接著設定每一個 host 的預設閘道器。
...
接著，為了路由器s2，將路由器s3的靜態路由指向 host (172.16.20.0/24)，但僅在 VLAN ID = 2的情況下。
...
從 h1s1 向 h1s3 發送 ping 訊息。因為是處於相同的 vlan_id=2 的相同 host ，且已經設置了指向s3的靜態路由在s2上，因此應該是可以正常連線的。
...
從 h2s1 向 h2s3 發送 ping 封包，雖然他們處於相同的 vlani_id=110 的 host ，但是路由器s2上並沒有設置指向路由器s3的靜態路由，因此無法成功連線。
