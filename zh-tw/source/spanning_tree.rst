...
本章將說明與解說 Ryu 所採用的擴充樹(Spanning Tree)安裝方法。


Spanning Tree
----------------

Spanning tree 是為了防止在網路的拓璞中出現迴圈(loop)進而產生廣播風暴(broadcast streams)的技術。
而且，藉著應用原本防止迴圈的這項功能，當網路發生問題的時候，則可以達到確保網路的拓璞被重新計算的目的，如此一來就不會讓部分的問題影響整個網路的連通。

Spanning tree 有許多的種類，例如：STP、RSTP、PVST+、MSTP...等不同的種類。本章將說明最基本的 STP。
...
STP 會在橋接器之間交換 BPDU(Bridge Protocol Data Unit)封包，分析及比對橋接器之間的連接埠訊息，決定哪些連接埠可以傳送，哪些不行。

具體來說，會以下列的順序完成。

1．Root 交換器(Root bridge)選舉

    橋接器之間的BPDU封包在交換過後，擁有最小的橋接器ID即為Root。
    接下來的 Root 橋接器會發送 original BPDU ，而其他的橋接器僅接收及轉發BPDU。


.. NOTE::

    橋接器ID的計算方式是組合已經被設定的橋接器 priority 和特定埠的 MAC address 而成。
...
2．決定埠的角色
    ...
    基於每一個埠到 Root 橋接器的距離來決定該埠的角色。

    * Root port

        橋接器內連接到 Root 距離最短的埠。該埠將會接收來自 Root 的 BPDU。

    * Designated port

        該埠從各個 link 到 Root 橋接器距離最短。
        主要轉送來自 Root 橋接器的 BPDU 封包，Root 橋接器的所有埠均為此種類。

    * Non designated port

        除了 Root port, designated port 以外的埠。
        訊框的傳送是被禁止的。
...
.. NOTE::
    ...
    每一個埠到 Root 橋接器的距離是基於收到 BPDU 中的設定值並加上下列的比較計算出來。
        ...
        第二優先：root path cost 相同的話，則比較橋接器ID

        第三優先：若是橋接器ID相同時(每一個埠都連接到相同的橋接器)、則比較連接埠ID
...
本章說明 「 `執行 Ryu 應用程式`_ 」中 OpenFlow 1.3 所對應的 spanning tree 應用程式(simple_switch_stp_13.py) 和 「 :ref:`ch_switching_hub` 」 的交換器之間的差異。
